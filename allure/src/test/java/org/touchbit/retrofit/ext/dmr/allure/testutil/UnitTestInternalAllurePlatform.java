 ...
 */
package org.touchbit.retrofit.ext.dmr.allure.testutil;
 ...
 */
public class UnitTestInternalAllurePlatform implements TestExecutionListener {
    ...
    public static void execute(String testID, Allure.ThrowableRunnableVoid runnable) {
        final TestIdentifier testIdentifier = getTestIdentifier();
        ...
        Allure.step(testID, runnable);
        testPlatform.executionFinished(testIdentifier, TestExecutionResult.successful());
    }

    public static TestIdentifier getTestIdentifier() {
        final UniqueId uniqueId = UniqueId.forEngine(UUID.randomUUID().toString());
        final TestEngineDescriptor engineDescriptor = new TestEngineDescriptor(uniqueId);
        return TestIdentifier.from(engineDescriptor);
    }
    ...
    public void executionSkipped(final TestIdentifier testIdentifier,
        ...
        reportNested(testPlan, testIdentifier, SKIPPED, new StatusDetails().setMessage(reason), new HashSet<>());
    }
    ...
    private void stopTestContainer(final TestIdentifier testIdentifier) {
        ...
        getLifecycle().updateTestContainer(uuid, container -> container.setChildren(new ArrayList<>(children)));
        getLifecycle().stopTestContainer(uuid);
        getLifecycle().writeTestContainer(uuid);
    }

    private void startFixture(final String parentUuid, final String type, final Map<String, String> keyValue) {
    ...
    private void startTestCase(final TestIdentifier testIdentifier) {
        ...
        testMethod.ifPresent(method -> ResultsUtils.getJavadocDescription(
    ...
    private void stopTestCase(final TestIdentifier testId, final Status status, final StatusDetails statusDetails) {
        final Optional<String> maybeUuid = tests.get(testId);
        if (!maybeUuid.isPresent()) {
            return;
        }
        final String uuid = maybeUuid.get();
    ...
    private Optional<Class<?>> getTestClass(final String className) {
        try {
            ...
            throw new RuntimeException("Could not get test class from test source " + className, e);
        }
    }
    ...
    private Optional<Method> getTestMethod(final MethodSource source) {
        try {
            ...
            throw new RuntimeException("Could not get test method from method source " + source, e);
        }
    }
