...
public class LoggingActionUnitTests {

    @Test
    @DisplayName("LoggingAction constructor without arguments")
    public void test1638279644171() {
        new LoggingAction();
    }

    @Test
    @DisplayName("#requestAction() logging an HTTP request")
    public void test1637857840571() throws IOException {
    ...
    public void test1638278910228() throws IOException {
        UnitTestLogger logger = new UnitTestLogger();
        final Response response = OkHttpUtils.getResponse();
        final Response result = new LoggingAction(logger).responseAction(response);
    ...
    public void test1638279095907() {
        UnitTestLogger logger = new UnitTestLogger();
        final RuntimeException exception = new RuntimeException("test1638279095907");
        new LoggingAction(logger).errorAction(exception);
        assertThat("", logger.getLogEventCount(), is(1));
        final SubstituteLoggingEvent logEvent = logger.getNextLogEvent();
